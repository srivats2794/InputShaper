...
for i = 1:samplePoints-1
    ...
    %using properties of bicycle model to calculate time delay filter
        ...
        deltaSW = A1*deltaSW_RAW_vec(i)+A2*deltaSW_RAW_history(end-round(t2/dt))+A3*deltaSW_RAW_history(end-round(t3/dt)); %here need time step duration 'dt' defined,
        %so we know which index of steering wheel history to access
        thetaSW_actual_3i = [thetaSW_actual_3i, deltaSW];
    
    delta= (1/20)*thetaSW_actual_3i(i);
